...
- **`private_key_jwt` audience** ([#1926]). The audiences accepted for a `private_key_jwt` client assertion are built only from the server's own identity — the `issuer` option or `Rails.application.routes.default_url_options[:host]` — and never from the request's `Host` header. A server that enables `private_key_jwt` without configuring either now refuses every assertion with `invalid_client` and logs an error at boot. Set one of the two and authentication resumes; no client-side change is needed. See the [migration guide](https://github.com/doorkeeper-gem/doorkeeper/wiki/Migration-from-old-versions#private_key_jwt-audience-is-built-from-the-servers-own-identity-1926) for details.
